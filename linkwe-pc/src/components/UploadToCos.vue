...
<template>
  <div>
    ...
    >
      ...
      <slot>
        <i v-if="!loading && !fileUrlWatch" class="el-icon-plus uploader-icon upload-action uploader-size"></i>

        <transition>
          <!-- 上传进度条 -->
          <div class="upload-action uploader-size" v-if="loading">
          ...
          <!-- 单文件上传的文件展示 -->
          <div v-if="!loading && fileUrlWatch && !multiple" class="upload-item">
            <template v-if="type === '0'">
              <el-image
                v-if="fileUrlWatch"
                ref="image"
