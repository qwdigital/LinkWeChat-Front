...
<template>
  <div class="add-sensitive-word">
    ...
    <pagination v-show="total > 0" :total="total" :page.sync="query.pageNum" :limit.sync="query.pageSize" @pagination="getSettingList()" />
    <!-- 添加敏感词 -->
    <el-dialog title="添加敏感词" :visible.sync="open" width="600px" append-to-body :close-on-click-modal="false">
      <el-form ref="addForm" :model="addForm" :rules="rules" label-width="130px">
        ...
        <el-form-item label="审计范围" :required="true" :error="rangeErrorMsg">
          <div class="tag-input" style="width: auto" @click="openAuditUserRange">
            <span class="tag-place" v-if="!auditUserRange.length">请选择
              <span class="prompt-title">若不选择，则默认全体成员</span>
            </span>
            <template v-else>
              <!-- <el-tag type="info" v-for="(unit, unique) in auditUserRange" :key="unique">{{ unit.name }}
              </el-tag> -->
              <el-tag type="info">{{ auditUserRange.map((data) => {return data.name}).join(',') }}
              </el-tag>
            </template>
          </div>
        </el-form-item>
        <el-form-item label="审计人" :required="true" :error="errorMsg">
          <div class="tag-input" style="width: auto" @click="openAuditUser">
            <span class="tag-place" v-if="!auditUser.length">请选择
              <span class="prompt-title">请选择部门负责人</span>
            </span>
            <template v-else>
              <el-tag type="info" v-for="(unit, unique) in auditUser" :key="unique">{{ unit.name }}
              </el-tag>
            </template>
          </div>
        </el-form-item>
    ...
    <!-- 选择审计范围弹窗 -->
    <SelectUser :defaultValues="auditUserRange" :isOnlyLeaf="false" :visible.sync="dialogVisibleSelectUser" title="选择添加人" @success="selectedUser"></SelectUser>
    <!-- 选择审计人弹窗 -->
    <SelectUser :visible.sync="dialogVisibleSelectAuditUser" title="选择添加人" isSigleSelect :defaultValues="auditUser" @success="selectedAuditUser"></SelectUser>
  </div>
</template>
<script>
      ...
      handleUpdate (row) {
        ...
        sensitiveApis.getSensitiveDetails(this.rowId).then((res) => {
          if (res.code === 200) {
            ...
            console.log(res.data.auditUserName)
            console.log(res.data.auditUserId)
            this.auditUser = [
              {
            ...
            if (res.data.auditUserScope[0].auditScopeName) {
              this.auditUserRange = []
              let arr1 = res.data.auditUserScope[0].auditScopeName.split(',')
              let arr2 = res.data.auditUserScope[0].auditScopeId.split(',')
