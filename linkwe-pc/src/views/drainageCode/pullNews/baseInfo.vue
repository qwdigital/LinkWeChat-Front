            ...
            <el-col :sm="24" :md="18">
              ...
              <el-form-item label="拉新方式:" required>
                <div class="function">
                  ...
                  >
                    <div>
                      <svg-icon class="icon-style" :iconClass="item.svg"></svg-icon>
                      <div>
                        <span>{{ item.name }}</span>
              ...
              <el-form-item label="添加标签:">
                <div class="sub-des">
                  通过扫描该二维码添加员工好友的新客户会自动打上此处设置的标签
                </div>
...
export default {
    ...
    query: {
      handler(val) {
        if (val.qrUserInfos) {
          ...
          this.$set(this.form, 'qrName', val.name)
  ...
  methods: {
    ...
    dealVal(data) {
      ...
      obj.executeUserCondit.change =
        data[0] && data[0].userIds && data[0].userIds.length ? true : false
      return obj
    },
    getExecuteData(data) {
    ...
    boxChange(item) {
      this.functionList.forEach((obj) => this.$set(obj, 'active', false))
      item.active = true
      this.$set(this.form, 'qrType', item.type)
    },
    handleValidateFn() {
      let status = null
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          status = this.$refs.selectMember.validateFn()
        } else {
          status = false
        }
      })
