<template>
  <div>
    <el-row :gutter="10" type="flex">
            ...
            <el-col :sm="24" :md="18">
              <el-form-item label="选择红包:" required v-if="query.qrType === 1">
                ...
                <div class="table-button">
                  ...
                  <el-button type="text" @click="radioClick(tableList)" class="button">{{
                    !tableList.length ? '选择红包' : '修改'
                  }}</el-button>
                </div>

                <div class="sub-des" v-if="!tableList.length">
                  选择红包之前需要在红包工具->企业红包中创建红包
                </div>
              </el-form-item>
              <el-form-item label="选择卡券:" required v-else>
                <!-- table -->
                <el-button type="text" @click="dialogVisibleCard = true">{{
                  !tableList.length ? '选择卡券' : '修改'
                }}</el-button>
                <div class="sub-des">
                  选择卡券之前需要在微信支付商户平台的营销中心提前创建代金券
                </div>
              </el-form-item>
              <IdentityAttachment
      ...
      <el-col style="width: 400px">
        <PreviewInPhone
          ...
          :pullnewsInfo="{ qrType: 1, materialName: tableList.length ? tableList[0].name : '' }"
        ></PreviewInPhone>
      </el-col>
    </el-row>
    <IdentitySelect
    ...
    <CardTable
      ...
      :twoVisible.sync="dialogVisibleCard"
      :qrType="query.qrType"
      :redId="redId"
...
export default {
  ...
  watch: {
    query: {
      handler(val) {
        ...
        if (val.attachments) {
          val.attachments.forEach((item) => {
            if (item.msgType === 'text') {
              ...
              this.$set(this.attachmentsData, 'templateInfo', item.content)
            } else {
              list.push(item)
            }
          })
          ...
          // this.$set(this.attachmentsData, 'attachments', val.attachments)
        ...
        if (list) {
          this.$set(this.attachmentsData, 'attachments', this.setAttachments(list))
  ...
  methods: {
    ...
    selectCard(val) {
      ...
      val.name ? this.$set(this.pullnewsInfo, 'materialName', val.name) : ''
      let obj = {
        businessId: val.id,
