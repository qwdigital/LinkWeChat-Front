  ...
  <div class="g-card g-pad20">
    <el-form :inline="true" label-width="80px" label-position="left">
      ...
      <el-form-item label="所属客服" prop="">
        <el-select v-model="search.kfId">
          <el-option value="" label="全部"></el-option>
          <el-option :value="unit.id" :label="unit.name" v-for="(unit, key) in customerList" :key="key"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange" style="margin-top:20px;width: 100%">
      ...
      <el-table-column label="场景分类" align="center" min-width="100" prop="typeName" show-overflow-tooltip />
      ...
      <el-table-column label="操作" align="center" fixed="right" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{ row }">
          <el-button type="text" class="copy_btn" :id="'btn' + row.id" :data-clipboard-text="row.kfUrl" :data-clipboard-target="'#' + 'btn' + row.id">复制链接</el-button>
            ...
            <el-select v-model="form.kfId" @change="setChange">
              <el-option style="height:50px;padding-top: 5px;" :value="unit.id" :label="unit.name" v-for="(unit, key) in customerList" :key="key">
                <div style="height: 40px;">
                  <show-customer-service :url='unit.avatar' :name='unit.name'></show-customer-service>
                </div>
              </el-option>
            </el-select>
          </el-form-item>
