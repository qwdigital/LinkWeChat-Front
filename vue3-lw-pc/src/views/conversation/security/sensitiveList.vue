...
<template>
  <div>
    <el-form :inline="true" :model="form" class="top-search">
    ...
    <div class="g-card">
      <el-table :data="tableData" stripe style="width: 100%;" :header-cell-style="{ background: '#fff' }">
        ...
        <el-table-column prop="status" label="消息状态">
          <!-- <template #header>
          {{ floorRange }}
          <el-select  v-model="floorRange" class="noborder" @change="chechName(floorRange)">
            <el-option v-for="item in displayOptions" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </template> -->
          <template #default="{ row }">
            <el-tag>{{ displayOptions[row.status].label }}</el-tag>
          </template>
        </el-table-column>
