...
<template>
  <div>
    <div class="mid-action">
      ...
      <div>
        <el-button :disabled="multiRealCode.length === 0" @click="handleBulkRemove">批量删除</el-button>
      </div>
    </div>
      ...
      <el-table-column prop="actualGroupQrCode" label="实际群码" align="center" width="80">
        <template #default="{ row }">
          <el-popover placement="bottom" trigger="hover">
            <template #reference><el-image :src="row.actualGroupQrCode" class="code-image--small"></el-image></template>
    ...
    <el-dialog title="实际群码" v-model="dialog" append-to-body>
      <el-form :model="form" ref="form" label-width="120px" :rules="rules">
        ...
        <el-form-item label="实际群码" prop="actualGroupQrCode">
          <upload v-model:fileUrl="form.actualGroupQrCode" :format="acceptType" class="image-uploader">
            <template #tip><div>注: 只支持2M以内的jpg/png格式图片</div></template>
          </upload>
          <div>
            <el-button text @click="exampleDialog = true">查看示例</el-button>
          </div>
        </el-form-item>
        <el-form-item label="有效期" prop="effectTime">
          <el-date-picker
            ...
            v-bind="datePickerOptions"></el-date-picker>
        </el-form-item>
      ...
      <el-dialog title="选择客户群" append-to-body v-model="customerDialog">
        <Customer
          v-if="customerDialog"
          ref="customer"
          :customerGroupId="form.chatId"
          @callback="getSelectCustomerGroup"></Customer>
      ...
      <el-dialog title="如图所示为标准的微信群二维码" v-model="exampleDialog" append-to-body width="400px">
        <div class="example-code-box">
          <!-- <div class="example-text"></div> -->
          <div class="code-box-title">
            <svg-icon icon="user" class="code-user"></svg-icon>
            实际群名称
          </div>
          <div class="code-content">
            <el-image :src="'@/assets/example/groupCode.png'"></el-image>
          </div>
        </div>
      </el-dialog>
    ...
    <pagination
      ...
      @pagination="getRealCodes"></pagination>
  </div>
</template>
