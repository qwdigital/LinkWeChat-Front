  ...
  <div>
    ...
    <el-form :inline="true" label-width="80px" label-position="left" class="top-search">
      <el-form-item label="客户名称">
        <el-input v-model="query.customerName" placeholder="请输入客户名称" clearable @keyup.enter="search()" />
      </el-form-item>
      <el-form-item label-width="120px" label="SOP结果状态">
      ...
      <el-form-item label="跟进员工">
        <el-input :model-value="userName" readonly @click="dialogVisible = true" placeholder="请选择跟进员工" />
      </el-form-item>
    ...
    <div class="g-card">
      <div class="g-card-title mid-action">
        数据明细
        <el-button type="primary" @click="syncFn()">同步</el-button>
      </div>

      <el-table v-loading="loading" :data="tableList" style="width: 100%">
        <el-table-column label="客户" align="center" min-width="100" prop="xtime" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="cp flex aic">
              <el-image style="width: 50px; height: 50px; flex: none" :src="row.avatar" fit="fill"></el-image>
              <div class="ml10">
                <p style="text-align: left">{{ row.customerName }}</p>
                <i :class="['el-icon-s-custom', { 1: 'man', 2: 'woman' }[row.gender]]" />
                <span :style="{ color: row.customerType === 1 ? '#4bde03' : '#f9a90b' }">
                  {{ { 1: '@微信', 2: row.corpName ? '@' + row.corpName : '@企业微信' }[row.customerType] }}
                </span>
              </div>
            </div>
          </template>
        ...
        <el-table-column
          ...
          min-width="100"
          show-overflow-tooltip></el-table-column>
        <el-table-column label="结束SOP时间" align="center" prop="executeEndTime" min-width="100" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.executeEndTime ? row.executeEndTime : '--' }}
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
          <template #default="{ row }">
            <el-button text :data="row" @click="getContent(row.weUserId, row.externalUserid, row.executeTargetId)">
              查看推送详情
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-drawer :size="650" title="查看推送详情" v-model="drawer" direction="rtl" :modal="false">
...
import {
  getDetailTab,
  getCustomerDetailTable,
  getCustomerSendContent,
  getSopDetail,
  getSyncData,
} from '@/api/sop/index'
...
export default {
  ...
  data() {
    return {
      ...
      cardData: [
        {
          title: 'SOP对象客户数',
          tips: '满足当前SOP执行的目标对象条件，且正在执行中的客户数',
          value: 0,
          title1: '较昨日',
          value1: 0,
        },
        {
          title: '正常结束客户数',
          tips: 'SOP执行完成后自动变更配置SOP结束条件的客户数',
          value: 0,
          title1: '较昨日',
          value1: 0,
        },
        {
          title: '提前结束客户数',
          tips: 'SOP执行过程中已满足SOP结束条件而提前结束SOP执行过程的客户数',
          value: 0,
          title1: '较昨日',
          value1: 0,
        },
        {
          title: '异常结束客户数',
          tips: '除了正常结束与提前结束外，不满足“员工继续发送”或“客户继续接收”导致的SOP无法正常执行而结束SOP的客户数',
          value: 0,
          title1: '较昨日',
          value1: 0,
        },
  ...
  methods: {
    ...
    getContent(weUserId, externalUserid, executeTargetId) {
      getCustomerSendContent({ weUserId, externalUserid, executeTargetId, sopBaseId: this.id }).then((res) => {
    ...
    getList() {
      ...
      getCustomerDetailTable(this.query).then((res) => {
