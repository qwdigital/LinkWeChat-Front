      ...
      <el-table v-loading="loading" :data="list" style="width: 100%">
        ...
        <el-table-column label="操作" align="center" fixed="right" width="200" class-name="small-padding fixed-width">
          <template #default="{ row }">
            ...
            <el-button text @click="goRoute('add', row.sopBaseId)">编辑</el-button>
            <el-button v-if="row.sopState === 2" text @click="updateStateFn(row.sopBaseId, 1)">执行</el-button>
            <el-button v-else text @click="updateStateFn(row.sopBaseId, 2)">暂停</el-button>
...
export default {
  ...
  methods: {
    ...
    removeFn(id) {
      this.$confirm('是否确认删除当前客群SOP？删除后不可撤销，历史数据将清空，请谨慎操作。', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
